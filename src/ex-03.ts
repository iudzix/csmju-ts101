...
// ฟังก์ชัน: ลบสมาชิกคนสุดท้าย (pop)
function removeLastMember(): Member | undefined {
    return BUS_MEMBERS.pop();
}

// ตัวอย่างการใช้งานฟังก์ชันลบสมาชิกคนสุดท้าย
const removed = removeLastMember();
if (removed) {
    console.log(`ลบสมาชิกคนสุดท้าย: ${removed.nameTH} (${removed.nameEN})`);
} else {
    console.log("ไม่มีสมาชิกให้ลบ");
}

// แสดงสมาชิกที่เหลือทั้งหมด
console.log("สมาชิกที่เหลือหลังจากลบ:");
BUS_MEMBERS.forEach(m => {
    console.log(`- ${m.nameTH} (${m.nameEN}), สูง ${m.heightCm} ซม., อายุ ${m.age} ปี`);
});
